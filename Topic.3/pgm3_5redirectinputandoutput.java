...
public class pgm3_5redirectinputandoutput
{
	public static void main(String[] args) throws IOException	//added throws clause
	{
		...
		//File myFile = new File("c:/BC/CISC1115/pgms/Topic3/myinput.txt");
		File myFile = new File("payroll_input.txt");		//myFile is a File reference variable
		...
		//can use forward slash in Java
		//PrintWriter outputFile = new PrintWriter("c:/BC/CISC1115/pgms/Topic3/myoutput.txt");
		PrintWriter outputFile = new PrintWriter("payroll_output.txt");
		...
		while (id != -1) {                   		//check for fake ID
			...
			outputFile.println();
			outputFile.printf("tax withheld was $%.2f leaving net pay of $%.2f", tax,netpay);
			outputFile.println();
			outputFile.println();
